...
def compute_covariance_matrix(returns_df): # should be actual returns up until prediction date
    # Reshape the data to have 'date' as index and 'permno' (stock identifier) as columns
    returns_pivot = returns_df.pivot(index='date', columns='permno', values='stock_exret')
    # Backfill any missing values (NaN) with 
...
# Calculate portfolio's expected return and volatility (risk)
...
def maximize_sharpe(weights, returns, cov_matrix, risk_free_rate=0.0):
    ...
    return -sharpe_ratio # optimize function minimizes so here's the negative sharpe

def stock_filter(predicted_returns) -> list: 
    # filter for top and bottom decile of stocks based on return predictions to include in our portfolio
...
def run(predicted_returns_df, actual_returns_df):
    ...
    # Reshape predicted returns to have 'date' as index and 'permno' as columns
    predicted_pivot = predicted_returns_df.pivot(index='date', columns='permno', values='predicted')
    optimal_weights_list = []

     # Process actual returns data
    ...
    for date in predicted_pivot.index:
        ...
        # Only use data up until the prediction date
        ...
        # Get predicted returns for the shortlisted stocks on the current date
        predicted_returns = predicted_pivot[shortlisted_stocks].loc[date].dropna()
        available_stocks = predicted_returns.index

         # Set up the initial weights and constraints for optimization
        ...
        # Store the optimal weights for each stock in the portfolio
